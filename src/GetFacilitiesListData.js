...
class GetFacilitiesListData extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  MobileGetUserConsentData() {
    let url =localStorage.getItem('url') + "/Getfacilities";
    console.log(url);
    ...
    fetch(url, {
      ...
      .then((result) => {
        console.log(result);
        this.setState({
            FacilityData:result["facilities"],
  ...
  render() {
    ...
    const { filter, FacilityData } = this.state;
    const lowercasedFilter = filter.toLowerCase();
    const filteredData= this.state.FacilityData.filter(item => {
      ...
      <div className={classes.root}>
        ...
        >
           <Grid  item xs={12} sm={12} style={{ marginBottom: "10px", }}>
                  <Typography variant="title" style={{ marginLeft: "15px",fontFamily: "Arial",fontWeight:"bolder" }}>
                  Please select your treatment clinic.
                  </Typography>
                  </Grid>
            <Card style={{ width: "98%" }}>
                <CardContent>
          <InputBase
            ...
            <Grid item xs={12} sm={12}>
                {filteredData!=""?filteredData.map((data,index)=>
                 ...
                 <ListItem 
                  button
                    onClick={e=>this.FacilityId(data.id)}
                 >
                      <img
...
export default withStyles(useStyles)(GetFacilitiesListData);
