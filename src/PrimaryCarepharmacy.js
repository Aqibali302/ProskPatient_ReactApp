...
class PrimaryCarepharmacy extends Component {
  constructor(props) {
    super(props);
    this.state = {
      physician: false,
  ...
  GetExistingData=()=>{
    ...
    .then((result) => {
      ...
      console.log(url);
      this.setState({
        PhysisanData:result["Pharmacies_list"],
  ...
  GetData=()=>{
      ...
      var zip=this.state.ZipCode.replaceAll("-","")
    let url=localStorage.getItem("url") +"/mobile/MobileGetCommonPharmaciesJSON?postal_code="+zip;
    fetch(url, {
      method: "POST",
      // body: JSON.stringify(data),
    })
  ...
  renderRadiusOptions() {
    return this.state.radius_dropdown.map((dt, i) => {
      return (
        <MenuItem key={i} value={dt.name}>
          {dt.name}
        </MenuItem>
  ...
  SelectedRadius = (event) => {
    this.setState({
        radius_val: event.target.value,
    });
   // this.GetData();
  };
        ...
        <Card style={{ width: "98%",margin:"auto" }}>
            ...
            <CardContent>
              <Grid container spacing={2}>
                ...
                >
                 Please select state and search your pharmacy from the list below
                </Typography>
                </Grid>
                <Grid item xs={12} sm={2}>
                  ...
                  <Grid item xs={12} sm={2}>
                    <FormControl
                      ...
                      className={classes.formControl}
                    >
                      <InputLabel htmlFor="state">Mile Radius</InputLabel>
                      <Select
                        IconComponent="false"
                        style={{ width: "100%" }}
                        value={this.state.radius_val}
                        onChange={this.SelectedRadius}
                        autoWidth
                        required
                        inputProps={{
                          name: "Radius",
              ...
              >
                ...
                />}</InputMask>
                  ...
                  <InputLabel htmlFor="standard-adornment-password">Pharmacy Name</InputLabel>
                  <Input
...
export default withStyles(styles)(PrimaryCarepharmacy);
